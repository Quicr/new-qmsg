...
namespace {
    ...
    TEST_F(DataBufferTest, ConstructorExistingBuffer)
    {
        ...
        qmsg::DataBuffer db(key, sizeof(key), sizeof(key));
    ...
    TEST_F(DataBufferTest, Copy2)
    {
        ...
        // Assign the DataBuffer to the local one
        qmsg::DataBuffer db(data_buffer);
    ...
    {
        qmsg::OctetString raw_data = { 0x01, 0x02, 0x03, 0x04 };

        data_buffer.AppendValue(raw_data);

        qmsg::DataBuffer db(std::move(data_buffer));
    ...
    TEST_F(DataBufferTest, SetBuffer)
    {
        ...
        std::uint8_t *p = new std::uint8_t[700];
    ...
    TEST_F(DataBufferTest, SetValueChar)
    {
        ...
        // Set the data length to something we can work with
        data_buffer.SetDataLength(16);
        data_buffer.SetValue(reinterpret_cast<const std::uint8_t *>(data),
                              4,
                              4);
    ...
    TEST_F(DataBufferTest, AppendValueChar)
    {
        ...
        data_buffer.AppendValue(reinterpret_cast<const std::uint8_t *>(data0),
                                4);
        ...
        data_buffer.AppendValue(reinterpret_cast<const std::uint8_t *>(data1),
                                4);
    ...
    TEST_F(DataBufferTest, ReadValue)
    {
        ...
        data_buffer.AppendValue(reinterpret_cast<const std::uint8_t *>(buffer),
                                5);
        ...
        // Read the data
        data_buffer.ReadValue(reinterpret_cast<std::uint8_t *>(read_buffer), 5);
